...
function verify($u, $data){
  ...
  curl_setopt_array($ch, array(
    ...
    $r = curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);

    // print_r($result);
    // print_r($http);
    // print_r($r);
    // header("Location: {$r}");
//     echo '<button onclick="Open()"><b>'.$data['budge'].'</b> '.$data['title'].' '.$data['view'].'</button>';
